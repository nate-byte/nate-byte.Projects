whenSquaresAttack.state5 = function(){};
whenSquaresAttack.state5.prototype = {
    count: {
        zombie: 0,
        powerUp: 0,
        numStars: 0,
    ...
    preload: function(){
        game.load.tilemap('state5map', 'assets/tilemaps/state5map.json',null,Phaser.Tilemap.TILED_JSON);
        ...
        game.load.image('endscreen', 'assets/buttons/endscreen.png');
    ...
    create: function(){
        ...
        // init map
        this.map = game.add.tilemap('state5map');
        ...
        this.sprite.crates.create(1200, 600, 'crate');
        this.sprite.crates.create(650, 400, 'crate');
        this.sprite.crates.create(700, 100, 'crate');

        // init stars: spawn 5 strategically
        this.sprite.stars = game.add.group();
        this.sprite.stars.create(1390, 240, 'star');
        this.sprite.stars.create(200, 560, 'star');
        this.sprite.stars.create(800, 100, 'star');
        this.sprite.stars.create(500, 940, 'star');
        this.sprite.stars.create(500, 250, 'star');

        // init text display
        game.add.text(CENTER_X, 10, 'Level 5', FONT_STYLE);
        ...
        //TODO: replace next level button; instead have You Win! screen pop up 
        ...
        this.button.endScreen = game.add.button(750,500,'endscreen', (function() {
            this.count.resetAll.call(this);
            this.currentPhase = PHASE.PLAYING;
            game.state.start('main-menu');
        }).bind(this));

        addButtonStylesTo(this.button.endScreen);
        this.button.endScreen.visible = false;
    ...
    update: function(){
        ...
        if (this.currentPhase === PHASE.PLAYING) {
            ...
            // zombies: all dead (wave 1: spawn 9 zombies; wave 2: go to ENDED)
            if ((this.count.zombie === 0) && !this.button.replay.visible) {
                if (this.count.wave === 1 && !this.isWaveTwoReady) {
                    this.isWaveTwoReady = true;
                    this.sprite.zombies.destroy(true, true);
                    this.time.events.add(Phaser.Timer.SECOND * 1, (function(){
                        var coordinateExceptions = [{
                            x: this.sprite.player.x,
                            y: this.sprite.player.y,
                        }];
                        for (var i = 0; i < 9; i++) {
                            var newCoordinate = getRandCoordinateExcept(1500, 1000, 64, coordinateExceptions);
                            coordinateExceptions.push(newCoordinate);
                        ...
                        game.physics.enable([this.sprite.zombies]);
                        this.sprite.zombies.setAll('body.collideWorldBounds', true);
                        this.count.wave += 1;
                    }).bind(this));
                } else if (this.count.wave == 2  && this.count.numStars == 5) {
                    this.currentPhase = PHASE.ENDED;
                    this.animation.stopAll.call(this);
    
                    this.button.play.visible = false;
                    this.button.pause.visible = false;
                    this.button.endScreen.visible = true;
